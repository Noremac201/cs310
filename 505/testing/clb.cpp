...
struct Node
{
                                ...
                                vector< uint > & calc )
    {
        ...
        uint before = lower_bound;
        if ( calc.at( jobpath.size() ) != UINT_MAX )
        {
            lower_bound += calc.at( jobpath.size() );
        }
        else
        {
            ...
            calc.at( jobpath.size() ) = lower_bound - before;
        }
    }
};
...
int main()
{
    ...
    node.calculate_lower_bound( cost, op_count, calc );
    for ( uint i = 0; i <= n; i++ )
        ...
        else
        {
            for ( uint i = 1; i <= n; i++ )
            {
                if ( !curnode.jobsused.at( i ) )
                {
                    ...
                    newnode.calculate_lower_bound( cost, op_count, calc );
